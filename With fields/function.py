...
def data_qubit_one(lista_J,dissipation,tfinal,N,Observavel,device="cpu"):
    """
    Simula a evolução temporal de um qubit sob a influência de um Hamiltoniano e possivelmente uma dissipação.
    
    Parâmetros:
        delta (float, opcional, padrão=1.0): Coeficiente multiplicador do operador σx no Hamiltoniano.
        dissipation (float, opcional, padrão=0): Taxa de dissipação aplicada ao qubit.
        tfinal (float, opcional, padrão=2): Tempo final da simulação.
        N (int, opcional, padrão=100): Número de pontos de tempo a serem simulados.
        device (str, opcional, padrão="cpu"): Dispositivo para onde os tensores do PyTorch serão movidos ("cpu" ou "cuda").
    
    Retornos:
        t_train (numpy.ndarray): Array de tempos discretos ao longo da simulação.
        y_train (torch.Tensor): Estados do qubit ao longo do tempo, convertidos para tensores PyTorch.
        expect (torch.Tensor): Valores esperados do operador σz ao longo do tempo.
        H (torch.Tensor): Hamiltoniano do sistema.
        O_op (torch.Tensor): Operador de medição σz.
    """

    options = qt.Options( store_states = True)
    H       = lista_J[0]* np.pi * qt.sigmaz() 
    c_ops   = [qt.sigmax()*np.sqrt(dissipation[0])]
  
    # Estado inicial
    psi0 = qt.basis(2, 0)

    # Lista de tempos
    t_train = np.linspace(0, tfinal, N)
    ...
    # Executar simulação
    result = qt.mesolve(H, psi0, t_train, c_ops, O_op,options=options)
    ...
    t_train = tc.tensor(t_train,dtype=tc.float16,requires_grad=True,
                         device=device)
    H       = tc.tensor(H.full(),dtype=tc.complex64,
                        device=device)
    O_op    = tc.tensor(O_op    ,dtype=tc.complex64,
                        device=device)
    c_ops   = tc.tensor(c_ops[0].full(),dtype=tc.complex64,
                        device=device)
    y_train = tc.tensor(y_train, requires_grad=True,
                        device=device)
    expect  = tc.tensor(expect, dtype=tc.float16,
                        device=device)

    return y_train, expect, H, O_op,c_ops

def data_qubit_two(lista_J,dissipation,tfinal,N,Observavel,device="cpu"):
    ...
    # Hamiltoniano
    H = 2*np.pi*lista_J[0]*sz1 + 2*np.pi*lista_J[1]*sz2 + 2*np.pi*lista_J[2]*sx1*sx2

    # Hamiltonian Lindbladian
    c_ops = [np.sqrt(dissipation[0])*qt.tensor(qt.sigmam(), qt.qeye(2)),
             np.sqrt(dissipation[1])*qt.tensor(qt.qeye(2), qt.sigmam())]
    ...
    O_op    = np.array( [op.full() for op in O_op],dtype=np.complex64 )
    C_op    = np.array( [op.full() for op in c_ops],dtype=np.complex64 )
    y_train = np.array( [ result.states[i].full().flatten() for i in range(N)],
                        dtype=np.complex64 )
    ...
    C_op    = tc.tensor(C_op, device = device)
    y_train = tc.tensor( y_train, device = device)
    expect  = tc.tensor( expect,device = device).transpose(0, 1)

    return y_train, expect, H, O_op,C_op

def data_jc(lista_J,dissipation,tfinal,N,Observavel,device="cpu"):
    # Parameters:
    ...
    psi0 = qt.tensor(qt.basis(Base_campo,0), qt.basis(2,1))
    
    # Operators
    a  = qt.tensor(qt.destroy(Base_campo), qt.qeye(2))
    sm = qt.tensor(qt.qeye(Base_campo)   , qt.destroy(2))
    N_op = a.dag()*a # Operador numero
    e_e = sm.dag()*sm
    
    # Hamiltonian Lindbladian
    #c_ops = []
    c_ops = [np.sqrt(dissipation[0])*a,
             np.sqrt(dissipation[1])*sm]
    # Hamiltonian
...
def data_qubit_two_crosstalk(lista_J,dissipation,tfinal,N,device="cpu"):
    # Operadores de Pauli para cada qubit
    primeiro_termo = qt.tensor(qt.sigmaz(),qt.qeye(2)) 
    Cross_talk = qt.tensor(qt.sigmax(), qt.sigmax())

    # Hamiltoniano
    H = 0.5*lista_J[0]*primeiro_termo  + lista_J[1]*Cross_talk
    ...
    # Estado inicial (cada qubit na superposição de |0> e |1>)
    # |+> = (|0> + |1>)/sqrt(2) 
    ket_plus = (qt.basis(2, 0) + qt.basis(2, 1))/np.sqrt(2)
    
    psi0 = qt.tensor(ket_plus, ket_plus)
    ...
    C_op    = tc.tensor(C_op, device = device)
    y_train = tc.tensor( y_train, device = device)
    expect  = tc.tensor( expect,device = device,dtype= tc.float).transpose(0, 1)
